const calculateAverage = (results, key, numberOfRuns) => {
  let total = 0;
  [...Array(numberOfRuns)].forEach(
    (_, index) => (total = total + results[index][key])
  );
  return total / numberOfRuns;
};

export const TestResults = ({ testInfo }) => {
  const averageInfo = {
  ...
  return (
    <div>
      ...
      <ul>
        <h3>Instructions</h3>
        <li>
          Check across tests for drastic differences, should be near identical.
        </li>
      ...
      <ul>
        ...
        <li>
          Standard deviation should only be a few milliseconds or something is
